...
const accounts = () =>
  db.accounts.map((a) => ({ ...a, orders: countOrdersOfBank(a.id) }));
export const listAccounts = accounts;
export function createAccount(input) {
  db.accounts.push({
    id: uid(), account_number: input.account_number, bank_name: input.bank_name,
    account_holder_name: input.account_holder_name, owner_user_id: null,
    is_active: true, created_at: now(),
  });
  return accounts();
}
export function setAccountActive(id, is_active) {
...
function countOrdersOfBank(bankId) {
  return db.orders.filter((o) => o.bank_account_id === bankId).length;
}
...
export function createOrder(input, actorId) {
  ...
  const o = {
    ...
    bank_account_id: input.bank_account_id, status: 'data_masuk',
...
export function updateStatus(id, to, actorId) {
  ...
  o.status = to;
  o.updated_at = now();
  if (to === 'proses_pick_up') o.picked_up_at = now();
...
export function scan(code, actorId) {
  ...
  o.picked_up_at = now();
  o.updated_at = now();
  pushEvent(o.id, actorId, 'picked_up', 'data_masuk', 'proses_pick_up', 'Scan nomor pesanan');
  return withMeta(o);
}
...
function seed() {
  ...
  const mk = (num, product, store, recipient, method, trader, bank, status, extra) => ({
  ...
  return { users, accounts, orders, photos, events, settings: { pending_threshold_hours: 3, min_photos: 1, max_photos: 3, max_file_mb: 20 } };
}
