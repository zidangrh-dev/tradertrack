...
export default (pool) => {
  ...
  const listOrders = async (query = {}) => {
    ...
    if (query.store?.length) { conds.push(`o.store_id = ANY($${vals.length + 1}::uuid[])`); vals.push(query.store); }
    if (query.product?.length) { conds.push(`o.product_id = ANY($${vals.length + 1}::uuid[])`); vals.push(query.product); }
  ...
  const clearBarcode = async (id) => {
    await pool.query(`UPDATE orders SET barcode_path = NULL, updated_at = now() WHERE id = $1`, [id]);
    return getOrder(id);
  };
